...
import { configureProfileColors } from '/js/profile-accent.js';
...
export const mount = async root => {
  ...
  try {
    ...
    configureProfileColors(profile);
    ...
    const showTelemetryPairingConfirm = () => {
      const dialog = document.createElement('div');
      dialog.className = 'profile-dialog';
      const removeDialog = () => { dialog.replaceChildren(); dialog.remove(); };
      dialog.innerHTML = '<section class="profile-dialog-panel"><h2>GENERATE TELEMETRY PAIRING CODE?</h2><p class="profile-note">This one-time code is valid for 10 minutes. Enter it in the VerseLink Telemetry Windows client. Generating a new code invalidates your previous unused code.</p><div class="profile-dialog-actions"><button type="button" data-action="cancel">CANCEL</button><button type="button" class="primary" data-action="generate">GENERATE PAIRING CODE</button></div></section>';
      document.body.append(dialog);
      dialog.querySelector('[data-action="cancel"]').onclick = removeDialog;
      dialog.querySelector('[data-action="generate"]').onclick = async event => {
        const button = event.currentTarget;
        button.disabled = true;
        try {
          const response = await fetch('/api/me/telemetry/pairing', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ schema: 1 }) });
          const body = await response.json().catch(() => ({}));
          if (!response.ok) {
            if (response.status === 429 || body.error === 'rate_limited') {
              const retryAfter = Number(response.headers.get('Retry-After'));
              throw Error(Number.isFinite(retryAfter) && retryAfter > 0 ? `Please try again in about ${Math.ceil(retryAfter / 60)} minute(s).` : 'Please try again later.');
            }
            if (response.status === 403 && body.error === 'account_inactive') throw Error('Your VerseLink account is not active.');
            throw Error('A pairing code could not be generated.');
          }
          if (body.schema !== 1 || typeof body.code !== 'string' || !/^(?:[0-9A-HJKMNP-TV-Z]{4}-){3}[0-9A-HJKMNP-TV-Z]{4}$/.test(body.code) || !Number.isFinite(Date.parse(body.expires_at))) throw Error('A pairing code could not be generated.');
          dialog.innerHTML = `<section class="profile-dialog-panel"><h2>PAIRING CODE CREATED</h2><code class="profile-secret" aria-label="One-time pairing code">${esc(body.code)}</code><p class="profile-note">Valid until ${esc(new Date(body.expires_at).toLocaleString())} (10 minutes). Enter this code in the VerseLink Telemetry Windows client. This code will not be shown again.</p><div class="profile-dialog-actions"><button type="button" data-action="copy">COPY PAIRING CODE</button><button type="button" data-action="done">DONE</button></div></section>`;
          const codeElement = dialog.querySelector('.profile-secret');
          dialog.querySelector('[data-action="copy"]').onclick = async copyEvent => { try { await copyText(codeElement.textContent); copyEvent.currentTarget.textContent = 'COPIED'; } catch { copyEvent.currentTarget.textContent = 'COPY FAILED'; } };
          dialog.querySelector('[data-action="done"]').onclick = removeDialog;
        } catch (error) {
          const message = error.message === 'Your VerseLink account is not active.' || error.message.startsWith('Please try again') ? error.message : 'A pairing code could not be generated.';
          dialog.innerHTML = `<section class="profile-dialog-panel"><h2>PAIRING CODE UNAVAILABLE</h2><p class="profile-error">${esc(message)}</p><div class="profile-dialog-actions"><button type="button" data-action="close">CLOSE</button></div></section>`;
          dialog.querySelector('[data-action="close"]').onclick = removeDialog;
        }
      };
    };
    ...
    const render = (message = '') => {
      ...
      root.querySelector('#profile-telemetry-pair').onclick = showTelemetryPairingConfirm;
