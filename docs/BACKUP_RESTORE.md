...
- Container: `blueprint-inventory-db`
- Database: `blueprints`
- User: `blueprints`

```powershell
docker exec blueprint-inventory-db pg_dump -U blueprints -d blueprints > blueprint-inventory.sql
```
...
```powershell
Get-Content .\blueprint-inventory.sql | docker exec -i blueprint-inventory-db psql -U blueprints -d blueprints
```
