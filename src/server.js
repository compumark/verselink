...
ALTER TABLE app_users ADD COLUMN IF NOT EXISTS text_color varchar(7);
DO $$ BEGIN
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'app_users_text_color_check') THEN
...
const getCurrentAppUser = async (req) => {
  ...
  const result = await pool.query(`SELECT u.id, COALESCE(u.verselink_name, u.display_name) AS display_name, u.display_name AS scmdb_display_name, u.verselink_name, u.is_admin, u.account_status, u.accent_color, u.text_color, c.user_handle, p.display_name AS profile_display_name FROM app_users u LEFT JOIN LATERAL (SELECT token_hash, user_handle FROM scmdb_connections WHERE app_user_id = u.id AND revoked_at IS NULL AND connection_status IN ('connected', 'pending') ORDER BY CASE connection_status WHEN 'connected' THEN 0 ELSE 1 END, last_seen_at DESC NULLS LAST LIMIT 1) c ON true LEFT JOIN scmdb_profiles p ON p.token_hash = c.token_hash WHERE u.id = $1`, [context.appUserId]);
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/mobiglass") {
      let html = applyVerseLinkBranding(await readFile(join(publicDir, "mobiglass.html"), "utf8"))
        ...
        .replace('<button data-view="trademax" title="TradeMax"><i class="nav-icon" style="--icon:url(\'/assets/icons/connection.svg\')"></i></button>', '<button data-view="missions" title="Missions" aria-label="Missions"><i class="nav-icon" style="--icon:url(\'/assets/icons/missions.svg\')"></i></button><button data-view="trademax" title="TradeMax"><i class="nav-icon" style="--icon:url(\'/assets/icons/connection.svg\')"></i></button>')
        .replace('<button class="app locked" type="button" data-app="trademax"><i class="app-icon" style="-webkit-mask-image:url(\'/assets/icons/connection.svg\');mask-image:url(\'/assets/icons/connection.svg\')"></i><span><strong>TRADEMAX</strong><small>Cargo Profit Optimizer</small></span><i class="lock"></i></button>', '<button class="app locked" type="button" data-app="missions"><i class="app-icon" style="-webkit-mask-image:url(\'/assets/icons/missions.svg\');mask-image:url(\'/assets/icons/missions.svg\')" aria-hidden="true"></i><span><strong>MISSIONS</strong><small>Plan and coordinate group objectives</small></span><i class="lock"></i></button><button class="app locked" type="button" data-app="trademax"><i class="app-icon" style="-webkit-mask-image:url(\'/assets/icons/connection.svg\');mask-image:url(\'/assets/icons/connection.svg\')"></i><span><strong>TRADEMAX</strong><small>Cargo Profit Optimizer</small></span><i class="lock"></i></button>')
        ...
        .replace("const views=new Set(['home','inventory','orders','groups','trademax','material','changelog']);", "const views=new Set(['home','inventory','orders','groups','missions','trademax','material','changelog','miningpool','about','admin','profile']);")
        ...
        .replace("else if(v==='changelog')changelog();else", "else if(v==='changelog')changelog();else if(v==='missions')import('/js/missions-mobiglass.js').then(module=>module.mount(content));else if(v==='orders')import('/js/orders-mobiglass.js').then(module=>module.mount());else if(v==='miningpool')import('/js/material-inventory-mobiglass.js').then(module=>module.mount());else if(v==='about')window.renderVerseLinkAbout(content,user);else if(v==='admin')import('/js/admin-mobiglass.js').then(module=>module.mount(content));else if(v==='profile')import('/js/profile-mobiglass.js').then(module=>module.mount(content));else")
...
.replace("</head>", '<style>.brand-home{text-decoration:none;color:inherit}.home-apps .app.locked{cursor:not-allowed}</style><script>(()=>{document.addEventListener("click",event=>{const material=event.target.closest(\'[data-app="material"],[aria-label="Open Material Inventory"]\');if(material&&document.body.dataset.sessionAuthenticated!=="true"){event.preventDefault();event.stopImmediatePropagation()}},true)})()</script><link rel="icon" type="image/png" href="/favicon.png?v=verselink"><link rel="apple-touch-icon" href="/assets/verselink.png"><link rel="manifest" href="/manifest.webmanifest"><script src="/js/about-mobiglass.js"></script><script type="module" src="/js/alias-mobiglass.js"></script><script type="module" src="/js/profile-mobiglass.js"></script><script type="module" src="/js/mobiglass-auth-entry.js"></script><script type="module" src="/js/notifications-mobiglass.js"></script><script type="module" src="/js/orders-core.js"></script><script type="module" src="/js/orders-mobiglass.js"></script><script type="module" src="/js/inventory-core.js"></script><script type="module" src="/js/inventory-mobiglass.js"></script><script type="module" src="/js/groups-core.js"></script><script type="module" src="/js/groups-mobiglass.js"></script><script type="module" src="/js/trading-core.js"></script><script type="module" src="/js/trading-mobiglass.js"></script><script type="module" src="/js/inventory-detail-clean.js"></script></head>');
      html = html.replace("</head>", '<style>html[data-theme=hector] .brand-mark{transform:none}.changelog-series{margin:20px 0 24px;padding:12px 0;border-top:1px solid var(--border);border-bottom:1px solid var(--border)}.changelog-series-label{display:block;margin-bottom:10px;color:var(--muted);font-size:10px;letter-spacing:.16em}.changelog-series-list{display:flex;gap:8px;flex-wrap:wrap}.changelog-series-button{padding:9px 12px;border:1px solid var(--border);background:var(--panel2);color:var(--text);font:inherit;letter-spacing:.06em;cursor:pointer}.changelog-series-button:hover,.changelog-series-button:focus-visible,.changelog-series-button.active{border-color:var(--accent);background:var(--accent);color:var(--bg);outline:0;box-shadow:0 0 12px var(--glow)}@media(max-width:600px){.changelog-series-list{flex-wrap:nowrap;overflow-x:auto;padding-bottom:4px}.changelog-series-button{flex:0 0 auto}}</style><script type="module" src="/js/version-watch.js"></script></head>');
      res.writeHead(200, { "content-type": "text/html; charset=utf-8", "cache-control": "no-store" });
      return res.end(html);
    }
    ...
    if (req.method === "POST" && url.pathname === "/api/profile") {
      ...
      const textInput = (params.get("text_color") || "").trim();
      const textColor = textInput === "" ? null : textInput.toUpperCase();
      ...
      if (textInput && !/^#[0-9A-F]{6}$/i.test(textInput)) return json(res, 400, { error: "text color must be a 6-digit hex value" });
      try {
        const updated = await pool.query("UPDATE app_users SET verselink_name=$1,rsi_profile_url=$2,discord_name=$3,profile_public=$4,accent_color=$5,text_color=$6,rsi_avatar_url=CASE WHEN rsi_profile_url IS DISTINCT FROM $2 THEN NULL ELSE rsi_avatar_url END,rsi_handle=CASE WHEN rsi_profile_url IS DISTINCT FROM $2 THEN NULL ELSE rsi_handle END,rsi_citizen_record=CASE WHEN rsi_profile_url IS DISTINCT FROM $2 THEN NULL ELSE rsi_citizen_record END,rsi_organization=CASE WHEN rsi_profile_url IS DISTINCT FROM $2 THEN NULL ELSE rsi_organization END,rsi_enlisted=CASE WHEN rsi_profile_url IS DISTINCT FROM $2 THEN NULL ELSE rsi_enlisted END,rsi_fluency=CASE WHEN rsi_profile_url IS DISTINCT FROM $2 THEN NULL ELSE rsi_fluency END,rsi_profile_synced_at=CASE WHEN rsi_profile_url IS DISTINCT FROM $2 THEN NULL ELSE rsi_profile_synced_at END WHERE id=$7 RETURNING id,COALESCE(verselink_name,display_name) AS display_name,verselink_name,display_name AS scmdb_display_name,rsi_profile_url,discord_name,profile_public,accent_color,text_color,rsi_avatar_url,rsi_handle,rsi_citizen_record,rsi_organization,rsi_enlisted,rsi_fluency,rsi_profile_synced_at", [name || null, rsiProfileUrl, discordName || null, params.get("profile_public") === "1", accentColor, textColor, current.id]);
        return json(res, 200, { ok: true, profile: { ...updated.rows[0], public_path: `/profile/${updated.rows[0].id}` } });
      } catch (error) {
        if (error.code === "23505") return json(res, 409, { error: "this VerseLink name is already in use" });
        throw error;
