  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/admin/state") {
      ...
      const [groups, users, tokenStats] = await Promise.all([
        pool.query(`SELECT g.id, g.name, g.created_at, COALESCE(json_agg(json_build_object('user_id',u.id,'name',COALESCE(u.verselink_name,u.display_name),'role',gm.role,'status',u.account_status) ORDER BY COALESCE(u.verselink_name,u.display_name)) FILTER (WHERE u.id IS NOT NULL),'[]') AS members FROM blueprint_groups g LEFT JOIN group_members gm ON gm.group_id=g.id LEFT JOIN app_users u ON u.id=gm.app_user_id GROUP BY g.id ORDER BY g.name`),
        pool.query("SELECT u.id, COALESCE(u.verselink_name, p.display_name, c.user_handle, u.display_name) AS display_name, u.is_admin, u.account_status, u.created_at FROM app_users u LEFT JOIN scmdb_connections c ON c.app_user_id=u.id LEFT JOIN scmdb_profiles p ON p.token_hash=c.token_hash ORDER BY display_name"),
